 ...
 *
 * ---------------------------------------------------------------------------
 * Why this is not the "hard-coded matrix" that `lib/formats.ts` forbids
 * ---------------------------------------------------------------------------
 *
 * `lib/formats.ts` says, in as many words, that there must never be a table of
 * the conversion matrix in this app, and `tests/matrix.test.tsx` exists to keep
 ...
 *      `lib/contract.ts`. Naming a target the contract does not define is a
 ...
 *      `tests/catalog.test.tsx`. Adding a page for a pair the service cannot
...
import type { TargetId } from "./contract";
 ...
 * `tests/catalog.test.tsx` asserts the grouping is lossless — that every
