...
export async function fetchFormats(signal?: AbortSignal): Promise<FormatsResponse> {
  const body = await requestJson("/formats", signal);
  return assertFormats(body);
}

/**
 * A shallow check that the body really is a matrix.
 *
 * The alternative is for every consumer to guard against `undefined.sources`
 * forever. This is the boundary; past it the types mean something.
 */
function assertFormats(body: unknown): FormatsResponse {
  if (typeof body !== "object" || body === null) {
    throw new ConversionFailed(unreadableResponseFailure(200, null));
  }
  const { sources, targets } = body as { sources?: unknown; targets?: unknown };
  if (!Array.isArray(sources) || !Array.isArray(targets)) {
    throw new ConversionFailed(unreadableResponseFailure(200, null));
  }
  return body as FormatsResponse;
}
