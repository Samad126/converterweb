 ...
 * (`lib/pdf/pdfCoords.ts`, unit-tested on its own).
 *
...
export function PdfPagePreview({
  ...
  useEffect(() => {
    ...
    return () => {
      cancelled = true;
      // Frees the worker and page memory; without it every file swap leaks one.
      void task?.destroy();
    };
  }, [file]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!context) return;
        const rendering = pdfPage.render({ canvasContext: context, viewport, canvas });
