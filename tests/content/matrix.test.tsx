...
import { formatList } from "@/lib/converter/formats";
...
describe("a matrix this build has never seen", () => {
  ...
  it("advertises the server's extensions and media types in the file input", async () => {
    ...
    // doesn't hide every file — see `acceptAttribute` in `lib/converter/formats.ts`.
    expect(
...
describe("no hard-coded matrix", () => {
  ...
  const GENERATED = "lib/api/api-types.ts";
   ...
   * literals outright, which is a good proxy for `lib/converter/formats.ts`,
   * `components/converter/FormatPicker.tsx` and everywhere else the UI reasons about
   * formats.
   *
   * `lib/content/catalog.ts` is the exception, and it is a deliberate one. It is the
   ...
   *
   *   - `tests/content/catalog.test.tsx` checks every extension and every pair in the
   ...
   */
  /**
   * `lib/content/catalog.ts` composes the prose for the thirty-six catalog pages, and
   ...
   * reason `lib/content/catalog.ts` is exempted for.
   */
  const EDITORIAL = new Set([
    ...
    // accepted extensions (`lib/api/pdfApi.ts`'s toolkit has no `GET /formats`
