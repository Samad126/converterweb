 ...
 *
 * `lib/api/api`'s `convert` is mocked here rather than driven through MSW. MSW
 ...
 * compressed ZIP — the same limitation `tests/api/transport.test.ts` notes for an
 ...
 * this environment cannot carry faithfully is replaced. `lib/converter/zip.ts`'s own
 * parsing of a real ZIP is exercised in full, without a mock, in
 * `tests/converter/zip.test.ts`.
 */
...
import type { ConvertHandle, ConvertResult } from "@/lib/api/api";
...
/** A resolved `ConvertHandle`, as `lib/api/api`'s real `convert` returns one. */
