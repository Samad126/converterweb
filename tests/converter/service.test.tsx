 ...
 * `tests/api/transport/nativeXhr.ts`. MSW's XHR interceptor has no `abort()`, so a
...
import { startServer, withNativeXhr } from "../api/transport/server";
...
describe("while a conversion is running", () => {
  it("stops saying Uploading once the body is on the wire", async () => {
    ...
    // the other half produces is covered in `tests/converter/progress.test.tsx`.
